...
void saveConfig(const JsonDocument &doc) {
  ...
  if (doc.containsKey("lora_sync")) {
    String syncStr = doc["lora_sync"] | "0x12";
    uint8_t syncVal = (uint8_t)strtol(syncStr.c_str(), NULL, 16);
    if (syncVal == 0 && syncStr != "0" && syncStr != "0x0" && syncStr != "0x00") {
